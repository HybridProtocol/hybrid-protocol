...
contract('configurableWeights', async accounts => {
  ...
  describe('Weights permissions, etc', () => {
    ...
    it('Non Controller account should not be able to change weights', async () => {
      await truffleAssert.reverts(crpPool.updateWeight(WETH, toWei('3'), { from: accounts[1] }), 'ERR_NOT_CONTROLLER');
    });
  ...
  describe('updateWeightsGradually', () => {
    ...
    it('Confirm update weights can be run with expired start block', async () => {
      ...
      console.log('Poking...');
      console.log(
        `${block.number} Weights: ${Decimal(fromWei(xyzWeight)).toFixed(4)} ${Decimal(fromWei(wethWeight)).toFixed(
          4,
        )} ${Decimal(fromWei(daiWeight)).toFixed(4)}`,
      );
