    ...
    describe('Presale LBP', () => {
      it('Should be able to update weights directly', async () => {
        ...
        for (i = 1; i <= 32000; i++) {
          ...
          console.log(
            `Admin balances after: ${Decimal(fromWei(adminUSDC)).toFixed(2)} USDC; ${Decimal(fromWei(adminHBT)).toFixed(
              2,
            )} HBT`,
          );
          ...
          console.log(
            `Pool balances after: ${Decimal(fromWei(poolUSDC)).toFixed(2)} USDC; ${Decimal(fromWei(poolHBT)).toFixed(
              2,
            )} HBT`,
          );
        }
