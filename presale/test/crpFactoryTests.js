...
contract('CRPFactory', async accounts => {
  ...
  it('should not be able to create with zero fee', async () => {
    ...
    await truffleAssert.reverts(crpFactory.newCrp(bFactory.address, poolParams, permissions), 'ERR_INVALID_SWAP_FEE');
  });
  ...
  it('should not be able to create with more than the max tokens', async () => {
    ...
    await truffleAssert.reverts(crpFactory.newCrp(bFactory.address, poolParams, permissions), 'ERR_TOO_MANY_TOKENS');
  });
});
