  ...
  describe('BAP0 shirt auction with mid-stream adjustment', () => {
    ...
    it('Should be able to poke weights (and buy shirts)', async () => {
      ...
      for (i = 0; i < 5; i++) {
        ...
        console.log(
          `Raw weights: ${Decimal(fromWei(weightBap0)).toFixed(4)} / ${Decimal(fromWei(weightDai)).toFixed(2)}`,
        );
        ...
        console.log(
          `User has ${Decimal(fromWei(daiBalance)).toFixed(2)} Dai and ${Decimal(fromWei(bap0Balance)).toFixed(
            1,
          )} shirts.`,
        );
        console.log(
          `Pool has ${Decimal(fromWei(tokenOutBalance)).toFixed(1)} shirts / ${Decimal(fromWei(tokenInBalance)).toFixed(
            2,
          )} Dai left`,
        );
    ...
    it('Should allow fast weight adjustment mid-stream', async () => {
      ...
      while (adjusting) {
        ...
        weightDai = await crpPool.getDenormalizedWeight(DAI);
        console.log(
          `Raw weights: ${Decimal(fromWei(weightBap0)).toFixed(4)} / ${Decimal(fromWei(weightDai)).toFixed(4)}`,
        );
